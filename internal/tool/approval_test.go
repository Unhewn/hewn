...
func TestPolicy_ReadOnlyNeverPrompts(t *testing.T) {
	...
	decision, err := p.Check(context.Background(), "read", RiskReadOnly, nil)
	if err != nil {
		t.Errorf("Check(read-only) = %v, want nil", err)
	}
	if decision != DecisionNotGated {
		t.Errorf("Check(read-only) decision = %v, want DecisionNotGated", decision)
	}
	if ap.calls != 0 {
...
func TestPolicy_Yolo(t *testing.T) {
	...
	decision, err := p.Check(context.Background(), "bash", RiskArbitrary, nil)
	if err != nil {
		t.Errorf("Check(yolo) = %v, want nil", err)
	}
	if decision != DecisionNotGated {
		t.Errorf("Check(yolo) decision = %v, want DecisionNotGated", decision)
	}
	if ap.calls != 0 {
...
func TestPolicy_AllowOnce(t *testing.T) {
	...
	if decision != DecisionAllowOnce {
		t.Errorf("Check #1 decision = %v, want DecisionAllowOnce", decision)
	}
	// Allow-once must not stick: the next call prompts again.
	if _, err := p.Check(context.Background(), "bash", RiskArbitrary, nil); err != nil {
...
func TestPolicy_AllowSessionPersists(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		if decision != DecisionAllowSession {
			t.Errorf("Check #%d decision = %v, want DecisionAllowSession", i, decision)
		}
	}
	if ap.calls != 1 {
...
func TestPolicy_DenyWithFeedback(t *testing.T) {
	...
	if decision != DecisionDeny {
		t.Errorf("Check() decision = %v, want DecisionDeny", decision)
	}
}
...
func TestPolicy_ApproverError(t *testing.T) {
	...
	_, err := p.Check(context.Background(), "bash", RiskArbitrary, nil)
...
func TestPolicy_ScopedPerTool(t *testing.T) {
	...
	if _, err := p.Check(context.Background(), "bash", RiskArbitrary, json.RawMessage(`{}`)); err != nil {
	...
	if _, err := p.Check(context.Background(), "write", RiskMutating, json.RawMessage(`{}`)); err != nil {
